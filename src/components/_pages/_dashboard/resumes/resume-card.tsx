...
export const ResumeCardButton = ({ title, description, icon }: ResumeCardButtonProps) => {
  return (
    <button
      className={cn(
        "w-full h-[300px] bg-muted/50 rounded border border-muted-foreground/20 flex items-center justify-center relative outline-none overflow-hidden",
...
const ResumeCard = () => {
  return (
    <Link href={'/dashboard/resumes/example'} className="block w-full">
      <ResumeCardButton title="Meu Currículo" description="Última atualização há 22 minutos" />
    </Link>
  );
}
